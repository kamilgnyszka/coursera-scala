...
abstract class TweetSet {
   ...
   */
    ...
    def mostRetweetedAcc(ts: Tweet): Tweet
   ...
   */
    def descendingByRetweet: TweetList = {
      def descendingBR(ts: TweetSet): TweetList = {
        if (ts.isInstanceOf[Empty]) Nil
        else {
          val t = ts.mostRetweeted
          new Cons(t, descendingBR(ts.remove(t)))
        }
      }
      descendingBR(this)
    }
...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Tweet = Nil.asInstanceOf[Tweet]

  def mostRetweetedAcc(ts: Tweet): Tweet = ts
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweetedAcc(ts: Tweet): Tweet = {
    ...
    if(elem.retweets > ts.retweets){
      val tl = left.mostRetweetedAcc(elem)
      val rt = right.mostRetweetedAcc(elem)
        if(tl.retweets > rt.retweets) tl else rt
    }
    else {
      val tl = left.mostRetweetedAcc(ts)
      val rt = right.mostRetweetedAcc(ts)
      if(tl.retweets > rt.retweets) tl else rt
    }
