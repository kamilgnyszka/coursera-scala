...
class ParallelCountChangeSuite extends FunSuite {
  ...
  test("countChange should return 0 for money < 0 (totalCoinsThreshold)") {
    def check(money: Int, coins: List[Int]) =
      assert(parCountChange(money, coins, totalCoinsThreshold(coins.size)) == 0,
        s"countChang($money, _) should be 0")
  ...
  test("countChange should return 1 when money == 0 (totalCoinsThreshold)") {
    def check(coins: List[Int]) =
      assert(parCountChange(0, coins, totalCoinsThreshold(coins.size)) == 1,
        s"countChang(0, _) should be 1")
  ...
  test("countChange should return 0 for money > 0 and coins = List() (totalCoinsThreshold)") {
    def check(money: Int) =
  ...
  test("countChange should work when there is only one coin (totalCoinsThreshold)") {
    def check(money: Int, coins: List[Int], expected: Int) =
  ...
  test("countChange should work for multi-coins (totalCoinsThreshold)") {
    def check(money: Int, coins: List[Int], expected: Int) =
      assert(parCountChange(money, coins,totalCoinsThreshold(coins.size)) == expected,
        s"countChange($money, $coins) should be $expected")
